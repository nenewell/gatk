...
public final class ExtractSAMRecordsByNameSpark extends GATKSparkTool {
    ...
    protected void runTool( final JavaSparkContext ctx ) {
        ...
        try ( final BufferedReader rdr =
                      new BufferedReader(new InputStreamReader(BucketUtils.openFile(readNameFile, getAuthenticatedGCSOptions()))) ) {
            String line;
            while ( (line = rdr.readLine()) != null ) {
                namesToLookFor.add(line.replace("@", "").replace("/1", "").replace("/2", ""));
            }
        }
        ...
        final Broadcast<HashSet<String>> namesToLookForBroadcast = ctx.broadcast(namesToLookFor);

        final Broadcast<SAMFileHeader> headerBroadCast = ctx.broadcast(getHeaderForReads());

        final JavaRDD<SAMRecord> x = getReads().repartition(80)
        ...
        logger.info("Found these many reads" + x.count());

        x.map(SAMRecord::getSAMString).coalesce(1).saveAsTextFile(outputFileName);
    }
}
